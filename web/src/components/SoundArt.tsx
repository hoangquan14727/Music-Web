...
export default function SoundArt({ image, alt = "", className = "" }: { image: string | null; alt?: string; className?: string }) {
  if (image) {
    ...
    return <img src={image} alt={alt} loading="lazy" suppressHydrationWarning draggable={false} className={`size-full object-contain ${className}`} />;
  }
  return (
    <span className={`relative grid size-full place-items-center ${className}`}>
      {/* eslint-disable-next-line @next/next/no-img-element */}
      <img src="/images/mascot/listening.webp" alt={alt} loading="lazy" suppressHydrationWarning draggable={false} className="size-3/4 object-contain opacity-80" />
